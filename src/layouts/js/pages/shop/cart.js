...
const func_a = (s, t, id, cb) => {
  ...
  htmlCoupon = (
    { discount: { fixed: fixedSale, percent: salePercent }, name, type },
    couponID,
    btnstate
  ) => `
		<li id="${couponID}">
        ...
        type == "shipment"
          ...
          : "$" + fixedSale
			...
			<p>${name}</p>
			<button data-state="${btnstate}">${btnstate == 0 ? "Apply" : "Remove"}</button>
		</li>`;

// @ts-ignore
function changeList(btnElem, couponID) {
	console.log(btnElem, couponID)
	...
	(([a, b, text]) => {
		func_a(cpns[a], cpns[b], couponID, () => {
			...
			// @ts-ignore
			document.getElementbyID(cpns[b])
...
user_coupon.applied_coupon_list.forEach((d, i) => {
  // @ts-ignore
  used_coupon.innerHTML += htmlCoupon(coupons_template[d], d, 1);
  // @ts-ignore
  const couponElem = document.getElementById(d),
    // @ts-ignore
    btn = couponElem.querySelector("button");
  // @ts-ignore
  couponElem.onclick = () => changeList(btn, d);
  // @ts-ignore
  btn.onclick = () => changeList(btn, d);
})
// @ts-ignore
user_coupon.available_coupon_list.forEach((d, i) => {
  // @ts-ignore
  available_coupon.innerHTML += htmlCoupon(coupons_template[d], d, 0);
  // @ts-ignore
  const couponElem = document.getElementById(d),
    // @ts-ignore
    btn = couponElem.querySelector("button");
